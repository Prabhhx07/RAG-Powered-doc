...
export default function DashboardPage() {
  ...
  const handleFile = async (file) => {
    ...
    try {
      ...
      setUploadSuccess(
        `"${file.name}" uploaded — ${res.chunks} chunks indexed.`,
      );
      await fetchDocs();
    } catch (err) {
  ...
  const handleDelete = async (e, docId, filename) => {
    e.stopPropagation(); // prevent the card's navigate() from firing
              ...
              >
                ...
                <div className="doc-card-body">
                  ...
                  <span className="doc-card-meta">
                    ID #{doc.document_id} · {formatDate(doc.created_at)}
                  </span>
                </div>
                <button
                  className="doc-card-delete"
                  onClick={(e) =>
                    handleDelete(e, doc.document_id, doc.filename)
                  }
